...
const { abi } = require(__dirname +
  "/artifacts/contracts/EvidenceManger.sol/EvidenceManager.json");
...
app.post("/addEvidence", async (req, res) => {
  ...
  async function storeDatainBlockchain(evidence) {
    ...
    const tx = await contract.addEvidence(
      ...
      0,
      evidence.location,
      evidence.caseDescription,
      0
    );
    await tx.wait();
  }
  await storeDatainBlockchain(evidence);
  res.json({ status: "Data addes to blockchain" });
});
